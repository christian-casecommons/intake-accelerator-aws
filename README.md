...
- `IntakeAcceleratorService` resource - ECS service that schedules containers configured in the `ApplicationTaskDefinition` task definition onto the `ApplicationCluster` ECS cluster, and manages service instance registration and deployments using the `ApplicationLoadBalancer` resource
